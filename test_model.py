import torch
import torch.nn as nn
import torchvision.models as models
import torchvision.transforms as transforms
from PIL import Image
import numpy as np
import cv2

# ==============================
# 🔹 Hybrid Model (same as training)
# ==============================
class HybridModel(nn.Module):

    def __init__(self):
        super().__init__()

        # Spatial branch (ResNet)
        self.cnn = models.resnet18(weights="DEFAULT")
        self.cnn.fc = nn.Identity()

        # Frequency branch (MobileNet)
        self.freq_model = models.mobilenet_v2(weights="DEFAULT")
        self.freq_model.classifier[1] = nn.Identity()

        # Fusion layer
        self.classifier = nn.Linear(512 + 1280, 2)

    def forward(self, spatial, frequency):
        spatial_feat = self.cnn(spatial)
        freq_feat = self.freq_model(frequency)

        combined = torch.cat((spatial_feat, freq_feat), dim=1)
        return self.classifier(combined)


# ==============================
# 🔹 Load Model
# ==============================
device = torch.device("cpu")

model = HybridModel()
model.load_state_dict(torch.load("hybrid_model.pth", map_location=device))
model.eval()

print("✅ Model loaded successfully")

# ==============================
# 🔹 Preprocessing (same as training)
# ==============================
transform = transforms.Compose([
    transforms.Resize((224, 224)),
    transforms.ToTensor()
])

# ==============================
# 🔹 Load Image
# ==============================
image_path = "test.jpg"   # keep test image in same folder

image = Image.open(image_path).convert("RGB")
spatial_img = transform(image).unsqueeze(0).to(device)

print("✅ Spatial image ready")

# ==============================
# 🔹 Create Frequency Image (FFT)
# ==============================
img_gray = cv2.imread(image_path, 0)

f = np.fft.fft2(img_gray)
fshift = np.fft.fftshift(f)

magnitude = 20 * np.log(np.abs(fshift) + 1)
magnitude = cv2.normalize(magnitude, None, 0, 255, cv2.NORM_MINMAX)

# Convert to 3-channel image
magnitude = np.stack((magnitude,)*3, axis=-1)

freq_img = Image.fromarray(magnitude.astype(np.uint8))
freq_img = transform(freq_img).unsqueeze(0).to(device)

print("✅ Frequency image ready")

# ==============================
# 🔹 Prediction
# ==============================
classes = ["fake", "real"]

with torch.no_grad():
    output = model(spatial_img, freq_img)
    probs = torch.nn.functional.softmax(output, dim=1)
    _, predicted = torch.max(output, 1)

confidence = probs[0][predicted.item()].item() * 100

print(f"🎯 Prediction: {classes[predicted.item()]} ({confidence:.2f}%)")
